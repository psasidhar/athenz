...
```
    cert-manager           approver-policy       athenz-issuer        Kubernetes API       Athenz ZTS          istiod
       (CM)                    (AP)                  (AI)                 (K8S)              (ZTS)              (ISO)
         ...
         │                      │                     │                     │               Derive instanceId from SA UID in token
...
```
    cert-manager           approver-policy       athenz-issuer        Kubernetes API       Athenz ZTS           ztunnel
       (CM)                    (AP)                  (AI)                 (K8S)              (ZTS)               (ZT)
         ...
         │                      │                     │  csr.DNS SAN: ztunnel.istio-system.svc.cluster.local
...
> ZTS derives `instanceId` from the SA UID in the validated K8s identity token when the CSR
> carries no `{uid}.instanceid.athenz` DNS SAN — no need to embed the SA UUID in the Certificate.
...
### Patch 5 — Fall back to SA-token instanceId when absent from CSR SANs (`InstanceUtils.java`)
Control-plane certs (istiod, ztunnel, istio-csr) carry hostname DNS SANs but no
`{sa-uid}.instanceid.athenz...` entry. ZTS derives `instanceId` from the K8s SA UID in the
validated identity token (`ZTSImpl.extractK8sSaUidFromAttestation`) and sets `ZTS_INSTANCE_ID`
on the `InstanceConfirmation` before calling the provider. The GCP K8s provider still called
`validateCertRequestSanDnsNames()`, which required the instance id to appear in DNS SANs.
The patch keeps existing behaviour when the instance id is present in CSR SAN DNS or SAN URI
(CSI and legacy flows). Only when extraction from the CSR fails does it fall back to
`ZTS_INSTANCE_ID` (server-side, derived from the validated SA token — not client-supplied).
Affects: istiod, ztunnel, and istio-csr control-plane certs.
